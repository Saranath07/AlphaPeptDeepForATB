...
This script implements uncertainty quantification for AlphaPeptDeep models using:
...
def run_rt_uncertainty_experiment(output_dir='results'):
    ...
    print("Running Deep Ensemble experiment...")
    ...
    fig = plot_calibration_curve(
        ...
        save_path=os.path.join(output_dir, 'deep_ensemble_calibration.png')
    )
    ...
    plt.grid(True)
    plt.savefig(os.path.join(output_dir, 'deep_ensemble_rt_predictions.png'))
    
    # Compare methods
    plt.figure(figsize=(12, 6))
    plt.scatter(mc_results['rt_norm'], mc_results['rt_pred_std'], alpha=0.5, label='MC Dropout')
    plt.scatter(ensemble_results['rt_norm'], ensemble_results['rt_pred_std'], alpha=0.5, label='Deep Ensemble')
    plt.xlabel('True RT')
    plt.ylabel('Prediction Standard Deviation')
    plt.title('Uncertainty Comparison: MC Dropout vs Deep Ensemble')
    plt.legend()
    plt.grid(True)
    ...
    ensemble_results.to_csv(os.path.join(output_dir, 'deep_ensemble_results.csv'), index=False)
    
    print(f"Results saved to {output_dir}")


def run_ms2_uncertainty_experiment(output_dir='results'):
